...
! VIIRS AOD code
  ...
  real(r_kind) :: rsst_txt, vaotq_txt   ! adicionado
...
!     call readmg(lunin,subset,idate,iret)
     ...
     open(lunin, file='modis.data', status="old", action="read", iostat=ios)
     read(lunin,*,iostat=ios1) subset, idate
     ...
     if ( ios1 == 0 ) then
        ...
        if (subset == 'NC008041') then
           ...
           read_modis: do
              ...
              write(*,*) ">>>>> Reading MODIS File <<<<<"
              read(lunin,*,iostat=ios2) rsat, slats0, slons0, solzen, azimuth,idate5(1),idate5(2),idate5(3),idate5(4),idate5(5), aod_550
              if (ios2 /= 0) exit
...
!     open(lunin,file=trim(infile),form='unformatted')   ! comentado
!     call openbf(lunin,'IN',lunin)                      ! comentado
!     call datelen(10)                                   ! comentado
!     call readmg(lunin,subset,idate,iret)               ! comentado

     open(lunin,file='viirs.data',status='old',action='read',iostat=ios)   ! adicionado
     read(lunin,*,iostat=ios1) subset, idate                                ! adicionado

!    if ( iret == 0 ) then
     if ( ios1 == 0 ) then                                                  ! adicionado
...
!          set valid range of AOD to ingest
           ...
           read_viirs: do
              irec=irec+1
!              call readsb(lunin,iret)                                    ! comentado
!              if (iret/=0) then                                          ! comentado
!                 call readmg(lunin,subset,jdate,iret)                    ! comentado
!                 if (iret/=0) exit read_viirs                            ! comentado
!                 cycle read_viirs                                        ! comentado
!              endif                                                      ! comentado
!
!              !    extract header information                            ! comentado
!              call ufbint(lunin,hdrvaodg,13,1,iret,vaodgstr)             ! comentado

              read(lunin,*,iostat=ios2) rsat, slats0, slons0, solzen, azimuth, &   ! adicionado
                                        rsst_txt, vaotq_txt, qcall, &               ! adicionado
                                        idate5(1), idate5(2), idate5(3), idate5(4), idate5(5), &  ! adicionado
                                        aod_550                                      ! adicionado
              if (ios2 /= 0) exit read_viirs                                         ! adicionado

              ksatid = nint(rsat)                                                    ! adicionado
!              rsat = hdrvaodg(1); ksatid=rsat                                        ! comentado
              ...
              !    Convert observation location to radians
!              slats0= hdrvaodg(2)                                                    ! comentado
!              slons0= hdrvaodg(3)                                                    ! comentado
              if(slons0< zero) slons0=slons0+r360
              if(slons0>=r360) slons0=slons0-r360
              dlat_earth_deg = slats0
              dlon_earth_deg = slons0
...
!              solzen  = hdrvaodg(9)                                                     ! comentado
!              azimuth = hdrvaodg(10)                                                    ! comentado

!              smask = zero                          ! over water                         ! comentado
!              if (nint(hdrvaodg(11)) > 0) then      ! over land                         ! comentado
!                 smask = one                        ! dark surface                       ! comentado
!                 call upftbv(lunin,"VAOTQ",hdrvaodg(12),mxib,ibit,nib)                  ! comentado
!                 if (nib > 0) then                                                      ! comentado
!                    if(any(ibit(1:nib) == 6)) then                                      ! comentado
!                       smask = two                  !  bright surface                   ! comentado
!                    endif                                                            ! comentado
!                 endif                                                               ! comentado
!              endif                                                                  ! comentado

              smask = zero                                                             ! adicionado
              if (nint(rsst_txt) > 0) smask = one                                     ! adicionado
              if (nint(vaotq_txt) == 6) smask = two                                   ! adicionado

!              qcall   = hdrvaodg(13)                                                 ! comentado

              !    Convert observation time to relative time
!              idate5(1) = hdrvaodg(4)  !year                                          ! comentado
!              idate5(2) = hdrvaodg(5)  !month                                         ! comentado
!              idate5(3) = hdrvaodg(6)  !day                                           ! comentado
!              idate5(4) = hdrvaodg(7)  !hour                                          ! comentado
!              idate5(5) = hdrvaodg(8)  !minute                                        ! comentado
...
!              call ufbrep(lunin,vaodch,2,12,iret,vaodchstr)                             ! comentado
!              aod_550 = vaodch(2,12)                                                    ! comentado
...
!       write(6,*)'READ_AEROSOL:  *** WARNING: read subset error, obstype=',obstype,', iret=',iret
        ...
        write(6,*)'READ_AEROSOL:  *** WARNING: read subset error, obstype=',obstype,', ios1=',ios1   ! adicionado
     end if
!     call closbf(lunin)   ! comentado
     close(lunin)
